...
class PointCloudOptimizer(BasePCOptimizer):
    ...
    def get_known_focal_mask(self):
        ...
        # All focals share the same requires_grad status
        return torch.tensor([not self.im_focals.requires_grad] * self.n_imgs)
    ...
    def get_depthmaps(self, raw=False):
        ...
        # Use inference_mode(False) to allow gradient computation in ComfyUI
        with torch.inference_mode(False):
            with torch.enable_grad():
                res = depthmaps.exp()
    ...
    def forward(self):
        ...
        # Ensure gradient computation is enabled (ComfyUI uses inference_mode)
        with torch.inference_mode(False):
            with torch.enable_grad():
